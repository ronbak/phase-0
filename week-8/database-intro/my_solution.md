...
10. sqlite> SELECT regions.region_name, states.state_name FROM regions INNER JOIN states ON regions.id=states.region_id ORDE
R BY states.region_id ASC;
...
Databases are for the storage of organized collections of information.

2. What is a one-to-many relationship?
A one-to-any relationship is where there is a table with 1 primary key that can be related another table where the primary key is now used as a foreign key and can appear many times.

3. What is a primary key? What is a foreign key? How can you determine which is which?
A primary key is a unique identifier in the table for each row. No two rows can share the same primary key information. A foreign key is a way to link two tables together.  A foreign key in one table points to the primary key in another table. A primary key can only appear once in the table, a foreign key can show many times in the table.

4. How can you select information out of a SQL database? What are some general guidelines for that?
You can select information out of an SQL database by choosing the column you want to view. To do this you would use the command: “SELECT column_name FROM table”. 